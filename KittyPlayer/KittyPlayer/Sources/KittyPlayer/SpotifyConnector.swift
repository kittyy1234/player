...
final class SpotifyConnector {
    ...
    private func getValidAccessToken(completion: @escaping (String?) -> Void) {
        ...
        guard let refresh = refreshToken else { completion(nil); return }
        var req = URLRequest(url: URL(string: "https://accounts.spotify.com/api/token")!)
        req.httpMethod = "POST"
        req.setValue("application/x-www-form-urlencoded", forHTTPHeaderField: "Content-Type")
        let body = [
        ...
        URLSession.shared.dataTask(with: req) { [weak self] data, _, _ in
            ...
            self.accessToken = access
            if let newRefresh = obj["refresh_token"] as? String { self.refreshToken = newRefresh }
            let expiresIn = obj["expires_in"] as? Double ?? 3600
            self.tokenExpires = Date().addingTimeInterval(expiresIn - 30)
            self.saveTokens()
            completion(access)
        ...
        func page(_ path: String) {
            apiGet(path) { obj in
                ...
                for p in items {
                    ...
                    if let url = images?.first?["url"] as? String { entry["image"] = url }
                    all.append(entry)
                }
                if let next = obj["next"] as? String, let range = next.range(of: "/v1") {
